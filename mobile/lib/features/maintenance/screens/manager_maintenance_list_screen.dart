...
class _ManagerMaintenanceListScreenState
  ...
  Widget _buildBody() {
    ...
    if (items.isEmpty) {
      return EmptyState(
        title: l10n.maintenanceNoRequests,
        subtitle: l10n.maintenanceNoRequestsSubtitle,
      );
    }
        ...
        itemBuilder: (_, i) {
          ...
          return Card(
            child: ListTile(
              title: Text(r.title),
              subtitle: Text(
                '${_categoryLabel(l10n, r.category)} • ${l10n.maintenancePriorityLabel}: $priLabel',
              ),
              trailing: StatusChip(
                kind: StatusKind.maintenance,
                value: r.status,
              ),
              onTap: () => context.push(
                '/manager/maintenance/${widget.buildingId}/${r.id}',
              ),
            ),
          );
        },
